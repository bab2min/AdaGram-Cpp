...
public:
	...
	AdaGramModel(AdaGramModel&& o)
		...
		code(std::move(o.code)), path(std::move(o.path))
	{
	}
	...
	void buildVocab(_Iter begin, _Iter end, size_t minCnt = 20, 
		_Pred test = [](const std::string&) { return true; })
	{
	...
	void train(const std::function<std::vector<std::string>(size_t, double&)>& reader, size_t numWorkers = 0,
		size_t window_length = 4, float start_lr = 0.025, size_t batchSents = 1000, size_t epoch = 1);
