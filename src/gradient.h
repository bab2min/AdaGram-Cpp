...
auto in_place_update(VectorModel& vm, int x, int y, std::vector<double>& z,
...
auto var_update_z(VectorModel& vm, int x, int y, 
...
auto var_update_counts(VectorModel& vm, int x, std::vector<double>& local_counts, double lr)
{
...
void inplace_train_vectors(VectorModel& vm, const std::vector<int32_t>& doc,
	...
	for (size_t i = 0; i < N; ++i)
	{
		...
		if (i % batch == 0)
		{
			...
			printf("%.2f%% %.4f %.4f %.4f %.2f/%.2f %.2f kwords/sec\n", 
				...
				(float)senses / i, max_senses, time_per_kword);
		}
