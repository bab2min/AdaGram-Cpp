...
int expected_logpi(std::vector<double>& pi, const VectorModel& vm, size_t w, float min_prob = 1e-3)
{
	float r = 0.f, x = 1.f;
	int senses = 0;
	std::copy(&vm.counts[{0, w}], &vm.counts[{0, w}] + vm.counts.size()[0], pi.begin());
...
int expected_pi(std::vector<double>& pi, const VectorModel& vm, size_t w, float min_prob = 1e-3)
{
	float r = 1.f;
	int senses = 0;
...
std::vector<double> expected_pi(const VectorModel& vm, size_t w, float min_prob = 1e-3)
{
